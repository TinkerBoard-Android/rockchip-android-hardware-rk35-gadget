...
#define LOG_TAG "librkusb-common"

#include "include/rkusb/CommonUtils.h"
...
int linkFunction(const char *function, int index) {
    ...
    if (kDebug)
        ALOGD("symlink %s -> %s", link, functionPath);
...
bool setVidPidCommon(const char *vid, const char *pid) {
    if (!WriteStringToFile(vid, VENDOR_ID_PATH)) {
        ALOGE("Failed to set vid: %s at %s, %s", vid, VENDOR_ID_PATH, strerror(errno));
        return false;
    }

    if (!WriteStringToFile(pid, PRODUCT_ID_PATH)) {
        ALOGE("Failed to set pid: %s at %s, %s", vid, PRODUCT_ID_PATH, strerror(errno));
        return false;
    }
